...
#
# Description:
#   Provides secure download functions with SHA256/SHA512 checksum verification.
...
#
# Functions:
#   - download_and_verify: Download file and verify checksum (SHA256/SHA512)
#   - download_and_extract: Download tarball, verify, and extract specific file
#   - verify_checksum: Verify existing file's checksum (SHA256/SHA512)
#
# Security:
#   - All downloads must provide expected checksum
#   - Supports SHA256 (64 hex) and SHA512 (128 hex) checksums
...
# Arguments:
#   $1 - URL to download
#   $2 - Expected checksum (SHA256 64 hex or SHA512 128 hex chars)
#   $3 - Output file path
#
# Returns:
...
download_and_verify() {
    ...
    if ! command curl -L -f --retry 8 --retry-delay 10 --retry-all-errors --progress-bar -o "$temp_file" "$url"; then
...
verify_checksum() {
    ...
    if [ "$checksum_len" -eq 40 ]; then
        echo -e "${RED}✗ SHA-1 checksums are not supported (cryptographically broken)${NC}" >&2
        echo "  Use SHA-256 (64 hex chars) or SHA-512 (128 hex chars) instead" >&2
        return 1
