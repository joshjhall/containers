export const meta = {
  ...
  description:
    'Budgeted, resumable fan-out for the live orchestrator over the OPEN-PR set: ' +
    '(1) polls PR CI/review state + issue status labels as authoritative, with a ' +
    'per-PR checkpoint so a crashed sweep resumes mid-list; (2) optionally classifies ' +
    'PRs behind base and dispatches the rebase-agent to auto-resolve trivial conflicts, ' +
    'surfacing the rest for the human. Both phases share ONE token budget. Never calls ' +
    'workflow() — the one Workflow nesting level is reserved for each golem process.',
  phases: [
    {
      title: 'Poll',
...
// "Conflict Classification for Cross-PR Rebase" decision tree. The first five
...
// Wraps the rebase-agent's native {resolved, escalated} output (the agent is
// unchanged — see agents/rebase-agent/rebase-agent.md).
const REBASE_RESULT = {
  type: 'object',
  additionalProperties: false,
  required: ['pr', 'branch', 'rebased', 'resolved', 'escalated'],
  properties: {
    pr: { type: 'integer' },
    branch: { type: 'string' },
...
const REBASE_GUARDRAILS =
  ...
  'live orchestrator pushes the rebased branch under human supervision. Escalate ' +
  'any logic / add-add / delete-modify conflict instead of guessing.'
...
const overlapPrompt = (pr) =>
  ...
  `or "has-logic" (any same-function, add-add, or delete-modify conflict). List ` +
...
const rebasePrompt = (pr, ov) =>
  ...
  `(${CONFLICT_CLASSES.slice(0, 5).join(' / ')}); escalate anything non-trivial. ` +
...
).filter(Boolean) // null-resilience: a failed PR read drops out, the sweep continues
    ...
    const [result] = await pipeline(
      ...
      (ov) => {
        ...
        if (!ov || ov.overlap === 'has-logic' || !ov.rebase_needed) {
          const files = ov ? ov.conflict_files : []
          return Promise.resolve({
            pr: pr.number,
            branch: pr.branch,
            rebased: false,
            resolved: [],
            escalated: files.map((f) => ({
              file: f,
              reason: ov && ov.overlap === 'has-logic' ? 'logic overlap — human review' : 'rebase not attempted',
            })),
          })
        }
