export const meta = {
  ...
  description:
    'Budgeted, resumable adversarial review for next-issue-ship: fans review ' +
    'dimensions (security/correctness/tests/conventions/scope-drift) as one ' +
    'parallel barrier under a single budget, folds in open PR review comments ' +
    '(post-PR cycles), re-scores with a fresh judge, then classifies each ' +
    'finding blocking-vs-deferrable for the skill to resolve-or-defer. One ' +
    'cycle per invocation — the skill owns the cycle loop and the cap.',
  phases: [
...
const classifyPrompt = (findings, budgetExhausted) =>
  ...
  (budgetExhausted
    ...
    : '') +
  `Return exactly one decision per finding, keyed by ref = ` +
...
reviewResults.forEach((res, i) => {
  if (!res) {
    log(`dimension "${dimensions[i].dim.name}" failed — continuing without its findings`)
    return
  }
