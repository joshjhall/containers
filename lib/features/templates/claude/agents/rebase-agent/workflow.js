...
const verifyPrompt = (file, cls, attempt) =>
  ...
  (cls.strategy === 'lockfile' || cls.strategy === 'generated'
    ? 'run the regeneration command (package manager / generator) for this file, then '
    : '') +
  `re-run the project's test/build to confirm the file is clean. Return ok=true ` +
  `only if it now passes; set flaky=true if the failure looks transient and a ` +
  `retry may help; otherwise flaky=false. ` +
  GUARDRAILS
