...
_verify_rc=0
verify_download "language" "python" "$PYTHON_VERSION" "$PYTHON_TARBALL" || _verify_rc=$?
...
if [ -d /tmp/python-project-files ] && [ -n "$(command ls -A /tmp/python-project-files 2>/dev/null)" ]; then
