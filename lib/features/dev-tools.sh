...
BIOME_VERSION="${BIOME_VERSION:-2.4.12}"
...
log_feature_summary \
    ...
    --tools "gh,lazygit,delta,act,git-cliff,glab,biome,taplo,uv,duf,entr,fzf,direnv,mkcert,jq,ripgrep,fd,bat,eza,htop,dua,lefthook,gitleaks,osv-scanner,hadolint,rumdl,dprint,typos,shfmt,conform,agnix,agentsys,cspell" \
    --paths "${DEV_TOOLS_CACHE},/opt/fzf,${CAROOT}" \
    --env "DEV_TOOLS_CACHE,CAROOT,DIRENV_ALLOW_DIR,ENABLE_LSP_TOOL" \
    --commands "gh,lazygit,delta,act,git-cliff,glab,biome,uv,uvx,duf,entr,fzf,direnv,mkcert,jq,rg,fd,bat,eza,htop,dua,lefthook,gitleaks,osv-scanner,hadolint,rumdl,dprint,typos,shfmt,conform,agnix,agentsys,cspell" \
