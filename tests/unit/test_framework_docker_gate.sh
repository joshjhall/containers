...
#
# Scrub rather than shim: a fake `docker` placed on PATH would still be found
# by `command -v`, which is the exact probe under test. Dropping every PATH
# component that holds an executable `docker` is what actually makes the probe
# fail. Coreutils are called by absolute path throughout, so losing a component
# cannot break the child.
# ---------------------------------------------------------------------------
docker_free_path() {
    local out="" dir
    local IFS=:
...
DOCKER_FREE_PATH="$(docker_free_path)"
...
# Sanity check: the scrubbed PATH really has no docker, or every assertion
# below would pass vacuously on a Docker-free host and prove nothing on a
# Docker-equipped one.
test_scrubbed_path_has_no_docker() {
    local out
    ...
    assert_contains "$out" "docker=absent" \
        "the scrubbed PATH must not resolve docker, or these tests prove nothing"

    # And the tools the child needs are still reachable.
    assert_not_empty "$DOCKER_FREE_PATH" "scrubbing PATH must not empty it"
}
...
run_test test_scrubbed_path_has_no_docker "scrubbed PATH really has no docker"
