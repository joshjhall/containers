...
test_rspec_config_loading() {
    ...
    if cp "$TEMPLATE_DIR/config/rspec.tmpl" "$tff_temp_dir/.rspec"; then
        if command grep -q -- "--require spec_helper" "$tff_temp_dir/.rspec" && command grep -q -- "--color" "$tff_temp_dir/.rspec"; then
...
test_rubocop_config_loading() {
    ...
    if cp "$TEMPLATE_DIR/config/rubocop.yml.tmpl" "$tff_temp_dir/.rubocop.yml"; then
        if command grep -q "AllCops:" "$tff_temp_dir/.rubocop.yml" && command grep -q "Metrics/MethodLength" "$tff_temp_dir/.rubocop.yml"; then
