...
setup() {
    local unique_id
    unique_id="$$-$(date +%s%N)"
    export TEST_TEMP_DIR="$TEST_SCRATCH_BASE/test-gpg-verify-terraform-$unique_id"
...
test_exports_download_and_verify_terraform_gpg() {
    assert_file_contains "$SOURCE_FILE" "protected_export.*download_and_verify_terraform_gpg" \
