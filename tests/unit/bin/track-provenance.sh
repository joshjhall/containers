#!/usr/bin/env bash
# Unit tests for bin/track-provenance.sh
...
# Test suite
test_suite "Track Provenance Tests"

# Path to script under test
SCRIPT="$(dirname "${BASH_SOURCE[0]}")/../../../bin/track-provenance.sh"

# ============================================================================
# Test: Script exists and is executable
# ============================================================================
test_script_exists() {
    assert_file_exists "$SCRIPT" "track-provenance.sh should exist"

    if [ -x "$SCRIPT" ]; then
        pass_test "track-provenance.sh is executable"
    else
        fail_test "track-provenance.sh is not executable"
    fi
}
...
test_help_output() {
    ...
    assert_contains "$output" "--repo" "Help should mention --repo"
}
...
test_script_references_tools() {
    ...
    assert_contains "$script_content" "cosign" "Should reference cosign"
    assert_contains "$script_content" "crane" "Should reference crane"
    assert_contains "$script_content" "rekor-cli" "Should reference rekor-cli"
}

# ============================================================================
# Test: Script has expected provenance functions
# ============================================================================
test_provenance_functions() {
    local script_content
    script_content=$(command cat "$SCRIPT")

    assert_contains "$script_content" "get_digest" "Should have get_digest function"
    assert_contains "$script_content" "check_signature" "Should have check_signature function"
    assert_contains "$script_content" "get_provenance" "Should have get_provenance function"
}
...
run_test test_provenance_functions "Provenance functions present"
