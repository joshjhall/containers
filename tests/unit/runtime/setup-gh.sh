...
test_auth_no_token_leak() {
    ...
    save_line=$(command grep -n '_xt=$(set +o | command grep xtrace)' "$SETUP_GH_SCRIPT" | command head -1 | command cut -d: -f1)
    disable_line=$(command grep -n '{ set +x; } 2>/dev/null' "$SETUP_GH_SCRIPT" | command head -1 | command cut -d: -f1)
    auth_line=$(command grep -n 'gh auth login --with-token' "$SETUP_GH_SCRIPT" | command head -1 | command cut -d: -f1)
    restore_line=$(command grep -n 'eval "$_xt"' "$SETUP_GH_SCRIPT" | command head -1 | command cut -d: -f1)
