...
if [ ! -S /var/run/docker.sock ]; then
    echo "ℹ️  No Docker socket found - skipping Docker GID configuration"
...
if [ -f "$ENV_FILE" ]; then
    ...
    if grep -q "^DOCKER_GID=" "$ENV_FILE"; then
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            command sed -i '' "s/^DOCKER_GID=.*/DOCKER_GID=$DOCKER_GID/" "$ENV_FILE"
        else
            command sed -i "s/^DOCKER_GID=.*/DOCKER_GID=$DOCKER_GID/" "$ENV_FILE"
        fi
        echo "✓ Updated DOCKER_GID in $ENV_FILE"
    else
        echo "" >> "$ENV_FILE"
        echo "# Docker socket access configuration (auto-detected)" >> "$ENV_FILE"
        echo "DOCKER_GID=$DOCKER_GID" >> "$ENV_FILE"
        echo "✓ Added DOCKER_GID to $ENV_FILE"
    fi
else
...
USER_GID=$(id -g)
EOF
    echo "✓ Created $ENV_FILE with Docker configuration"
fi
