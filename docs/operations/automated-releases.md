...
1. Install the Pushover app on your phone/desktop
1. Create a new application for this repository:
...
1. The `auto-patch/*` branch is preserved
1. Review the branch and failed workflow
1. Fix issues manually or delete the branch
1. The next scheduled run will try again
...
1. **Automatic validation** - Full CI runs before merge
1. **Fast notifications** - Know immediately if something breaks
1. **Preserved branches** - Failed patches kept for review
1. **Audit trail** - Version tags and commit history maintained
...
1. Test Pushover manually: `https://pushover.net/`
...
1. Verify workflow is enabled in GitHub Actions
1. Manually trigger to test: `gh workflow run auto-patch.yml`
