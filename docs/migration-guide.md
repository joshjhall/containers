...
```text
v3.x → v4.0.0 → v4.15.x (current)
v4.0.x → v4.15.x (direct upgrade)
v4.1.x → v4.15.x (direct upgrade)
v4.5.x → v4.15.x (direct upgrade)
```
...
1. Check feature dependencies:
   ...
   ```bash
   cat containers/docs/reference/features.md
   ```
