...
This document lists all environment variables used in the container build system, organized by category.
...
Build arguments are passed during `docker build` and control what gets installed. They are converted to environment variables during the build process.
...
| `INCLUDE_OP_CLI` | Install 1Password CLI |
...
- [Troubleshooting](troubleshooting.md) - Common issues with environment variables
