...
**Why:** stibbons needs Windows support because it runs on the host (not
just in containers). igor and luggage are container-only so don't need
Windows.

**How to apply:** Windows rust-test in CI is now a hard requirement (no
`continue-on-error`). If octarine regresses on Windows, the CI will catch
it.
