...
#
# Handles first-time setup: git hooks, dependency fetching. Called by
# agent-entrypoint.sh when the agent-initialized marker doesn't exist yet.
#
# Environment (set by entrypoint):
#   AGENT_NUM      — agent number (1-N)
#   AGENT_SUFFIX   — agent suffix (agent01, agent02, etc.)
#   AGENT_REPOS    — comma-separated repo names (set by stibbons)
...
# ============================================================================
# Worktrees share .git/hooks with the main repo. Configure each worktree to use
# the existing hooks from its main repo.
